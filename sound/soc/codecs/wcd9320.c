				   ...
				   const struct kernel_param *kp)
{
	...
	if (!priv) {
		pr_info("%s: codec isn't yet registered\n", __func__);
		return 0;
	}
					...
					struct snd_ctl_elem_value *ucontrol)
{
	...
	pr_info("%s: IIR #%d band #%d enable %d\n", __func__,
		iir_idx, band_idx,
			       ...
			       struct snd_ctl_elem_value *ucontrol)
{
	...
	if (comp == COMPANDER_1 &&
		...
		pr_info("%s: Enabled Chopper and set wavegen to 5 msec\n",
				__func__);
		...
		pr_info("%s: Disabled Chopper and set wavegen to 20 msec\n",
				__func__);
	}
	return 0;
}
	...
	struct snd_ctl_elem_value *ucontrol)
{
	...
	if (strnstr(taiko_conn_mad_text[taiko_mad_input],
		...
		pr_info("%s: taiko amic input widget = %s\n", __func__,
	...
	pr_info("%s: taiko input widget = %s\n", __func__, mad_input_widget);
	...
	if (mic_bias_found) {
		pr_info("%s: source mic bias = %s. sink = %s\n", __func__,
			     ...
			     struct snd_ctl_elem_value *ucontrol)
{
	...
	pr_info("%s: name %s sname %s updated value %u shift %d\n", __func__,
		...
		struct snd_kcontrol *kcontrol, int event)
{
	...
	case SND_SOC_DAPM_POST_PMD:
		...
		pr_info("%s: sleeping 5 ms after %s PA turn off\n",
				__func__, w->name);
				     ...
				     struct snd_kcontrol *kcontrol, int event)
{
	...
	pr_info("%s: spkr_pa_widget_on = %d\n", __func__, taiko->spkr_pa_widget_on);
	return 0;
}
	...
	struct snd_kcontrol *kcontrol, int event)
{
	...
	case 2:
		...
		pr_info("%s() event %d DMIC%d dmic_1_2_clk_cnt %d\n",
	...
	case 4:
		...
		pr_info("%s() event %d DMIC%d dmic_3_4_clk_cnt %d\n",
			__func__, event,  dmic, *dmic_clk_cnt);
		break;
	...
	case 6:
		...
		pr_info("%s() event %d DMIC%d dmic_5_6_clk_cnt %d\n",
			__func__, event,  dmic, *dmic_clk_cnt);
...
static void tx_hpf_corner_freq_callback(struct work_struct *work)
{
	...
	pr_info("%s(): decimator %u hpf_cut_of_freq 0x%x\n", __func__,
	...
	struct snd_kcontrol *kcontrol, int event)
{
	...
	pr_info("%s(): widget = %s dec_name = %s decimator = %u\n", __func__,
			      ...
			      struct snd_kcontrol *kcontrol, int event)
{
	...
	case SND_SOC_DAPM_POST_PMU:
		...
		pr_info("%s: sleep %d us after %s PA enable\n", __func__,
	...
	case SND_SOC_DAPM_POST_PMD:
		usleep_range(pa_settle_time, pa_settle_time + 1000);
		pr_info("%s: sleep %d us after %s PA disable\n", __func__,
				pa_settle_time, w->name);
		...
		struct snd_soc_dai *dai)
{
	...
	pr_info("%s(): dai AIF widget = %s. dai playback stream_name = %s.\n"
...
int taiko_mclk_enable(struct snd_soc_codec *codec, int mclk_enable, bool dapm)
{
	...
	pr_info("%s: mclk_enable = %u, dapm = %d\n", __func__, mclk_enable,
		 dapm);
...

{
	...
	case AIF3_PB:
		...
		list_for_each_entry(ch, &taiko_p->dai[dai->id].wcd9xxx_ch_list,
				    list) {
			pr_info("%s: slot_num %u ch->ch_num %d\n",
	...
	u8 tx_fs_rate_reg_val, u32 sample_rate)
{
	...
	list_for_each_entry(ch, &taiko->dai[dai->id].wcd9xxx_ch_list, list) {
		...
		if (decimator) { /* SLIM_TX port has a DEC as input */
			...
			pr_info("%s: set DEC%u (-> SLIM_TX%u) rate to %u\n",
			...
			if ((tx_port_reg_val >= 0x1) &&
				...
				pr_info("%s: RMIX%u going to SLIM TX%u\n",
				...
				pr_info("%s: no signal to SLIM TX%u\n",
					__func__, tx_port);
			} else {
				     ...
				     int event)
{
	...
	pr_info("%s: w->name %s w->shift %d event %d\n",
				...
				int event)
{
	...
	pr_info("%s(): w->name %s event %d w->shift %d\n",
...
static irqreturn_t taiko_slimbus_irq(int irq, void *data)
{
	...
	for_each_set_bit(j, &status, 32) {
		...
		if (val & TAIKO_SLIM_IRQ_PORT_CLOSED) {
			 ...
			 */
			bit = (tx ? j - 16 : j + 16);
			pr_info("%s: %s port %d closed value %x, bit %u\n",
				 __func__, (tx ? "TX" : "RX"), port_id, val,
				 bit);
			for (k = 0, cleared = false; k < NUM_CODEC_DAIS; k++) {
				pr_info("%s: priv->dai[%d].ch_mask = 0x%lx\n",
...
static ssize_t normalized_headphone_gain_show(struct kobject *kobj,
...
static ssize_t normalized_headphone_gain_store(struct kobject *kobj,
...
static ssize_t normalized_speaker_gain_show(struct kobject *kobj,
		struct kobj_attribute *attr, char *buf)
{
	int spkval;
...
static ssize_t normalized_speaker_gain_store(struct kobject *kobj,
		struct kobj_attribute *attr, const char *buf, size_t count)
{
		...
		struct kobj_attribute *attr, char *buf)
{
	int leftval;
	int rightval;
	leftval = wcd9xxx_reg_read(sound_control_codec_ptr, TAIKO_A_CDC_RX1_VOL_CTL_B2_CTL);
	rightval = wcd9xxx_reg_read(sound_control_codec_ptr, TAIKO_A_CDC_RX2_VOL_CTL_B2_CTL);
	return sprintf(buf, "%d %d\n", leftval, rightval);
}
...
{
	int spkval;
	spkval = wcd9xxx_reg_read(sound_control_codec_ptr, TAIKO_A_CDC_RX7_VOL_CTL_B2_CTL);
	return sprintf(buf, "%d\n", spkval);
}
...
static struct kobj_attribute normalized_headphone_gain_attribute =
	__ATTR(normalized_headphone_gain, 0644,
		normalized_headphone_gain_show,
		normalized_headphone_gain_store);

static struct kobj_attribute normalized_speaker_gain_attribute =
	__ATTR(normalized_speaker_gain, 0644,
		normalized_speaker_gain_show,
		normalized_speaker_gain_store);
...
static struct kobj_attribute sound_hw_revision_attribute =
	__ATTR(sound_control_hw_revision, 0444,
...
static struct attribute *sound_control_attrs[] = {
		...
		&normalized_&headphone_gain_attribute.attr,
		&normalized_&speaker_gain_attribute.attr,
...
static int taiko_codec_probe(struct snd_soc_codec *codec)
{
	...
	sound_control_codec_ptr = &control->core_res;
